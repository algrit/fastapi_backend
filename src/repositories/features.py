...
class RoomsFeaturesRepository(BaseRepository):
    ...
    async def update_rooms_features(self, room_id: int, new_features: list[int]):
        ...
        if features_to_delete:
            features_m2m_delete_stmt = delete(RoomsFeaturesORM).filter(
        ...
        if features_to_add:
            features_m2m_add_stmt = insert(RoomsFeaturesORM).values(
                [{"room_id": room_id, "feature_id": f_id} for f_id in features_to_add]
            )
